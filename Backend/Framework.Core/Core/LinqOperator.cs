...
namespace Framework.Core.Core
{
    public class LinqOperator<TEntity> : ILinqOperator<TEntity>
    {
            ...
            bool isNegation = false)
        {
            ...
            //var sort = propertyName.Substring(propertyName.IndexOf(" ") + 1);
            //propertyName = propertyName.Substring(0, propertyName.IndexOf(" "));

            PropertyInfo property = null;
            Type propertyType = null;

            foreach (var item in propertyName.Split('.').ToList())
            {
                if (property == null)
                {
                    property = _context.DataSetType.GetProperty(item.FirstCharToUpper());
                      propertyType = property.PropertyType;
                    if (propertyType.GetProperty("Value") != null)
                    {
                        property = propertyType.GetProperty("Value");
                ...
                else
                {
                    ...
                    property = propertyType.GetProperty(item.FirstCharToUpper());
                    propertyType = property.PropertyType;
                    if (propertyType.GetProperty("Value") != null)
                    {
                        property = propertyType.GetProperty("Value");
        ...
        private static bool IsPropertyTypeAndFilterMatchModeValid(Type propertyType, string extensionMethod)
        {
            ...
            {
                var validStringLinqMethods = new[] { LinqOperatorConstants.ConstantEquals };
                return validStringLinqMethods.Contains(extensionMethod);
            }
